...
func _register_all() -> void:
	...
	_add_joy_button(&"move_left", JoyButton.DPAD_LEFT)
	_add_joy_button(&"move_right", JoyButton.DPAD_RIGHT)
	_add_joy_button(&"move_up", JoyButton.DPAD_UP)
	_add_joy_button(&"move_down", JoyButton.DPAD_DOWN)
	_add_joy_axis(&"move_left", JoyAxis.LEFT_X, -1.0)
	_add_joy_axis(&"move_right", JoyAxis.LEFT_X, 1.0)
	_add_joy_axis(&"move_up", JoyAxis.LEFT_Y, -1.0)
	_add_joy_axis(&"move_down", JoyAxis.LEFT_Y, 1.0)

	# Jump / parry / dodge — Xbox-like layout as default reference
	_add_key(&"jump", KEY_SPACE)
	_add_joy_button(&"jump", JoyButton.A)

	_add_key(&"parry", KEY_E)
	_add_joy_button(&"parry", JoyButton.LEFT_SHOULDER)

	_add_key(&"dodge", KEY_SHIFT)
	_add_key(&"dodge", KEY_Z)
	_add_joy_button(&"dodge", JoyButton.B)
